...
### WMX3 제어기 사용 시
- SoftServo WMX3 SDK 설치 (`C:\Program Files\SoftServo\WMX3`)
- `WMX3ApiCLR.dll`, `IMDll.dll` 등 SDK DLL이 필요합니다
- Post-build 이벤트에서 `IMDll.dll`을 출력 폴더로 복사합니다

```batch
if exist "C:\Program Files\SoftServo\WMX3\lib\IMDll.dll" (
    copy /y "C:\Program Files\SoftServo\WMX3\lib\IMDll.dll" "$(TargetDir)"
) else (
    echo [WMX3] IMDll.dll not found - skipping copy
)
```

### Ajin AMP 제어기 사용 시
- `AXL.dll`을 빌드 출력 폴더(`bin\x64\Debug\` 등)에 배치
- EzSoftware UC SDK: `C:\Program Files (x86)\EzSoftware UC\AXL(Library)\Library\64Bit\`

### NI-DAQ 사용 시
- NI-DAQmx 드라이버 설치
- NI MAX에서 디바이스 이름 확인 후 `MainForm.cs`의 `DaqModule` 상수 수정
...
### 하드웨어 없이 실행 (시뮬레이션 모드)
별도 SDK나 하드웨어 없이도 실행 가능합니다. 앱 시작 시 **"시뮬레이션 (하드웨어 없음)"** 을 선택하면 모든 모션/DAQ 동작이 로그만 남기고 실행됩니다.
...
```bash
git clone https://github.com/<your-username>/PHM-Motion-Studio.git
cd PHM-Motion-Studio
```

Visual Studio에서 `PHM_Project_DockPanel.sln`을 열고 **x64 / Debug** 구성으로 빌드합니다.

> NuGet 패키지는 빌드 시 자동 복원됩니다.
...
Python 추론 서버(별도 레포)와 연동합니다. `MainForm.cs`에서 서버 주소를 수정하세요.
